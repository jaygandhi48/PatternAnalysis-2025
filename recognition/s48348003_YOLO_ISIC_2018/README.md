...
Since there can be multiple grid predictions for a given object, not all may be relevant or necessary. Hence the technique of Intersection Over Union (IoU) aims to filter these. It selects images that are above a certain IoU threshold.
...
In ISIC2018 dataset, the ground truth masks are segmentation masks, showcasing the skin lesion as white in colour and the background as black. However, this is not acceptable format as for detection and classification, bounding boxes need to be calculated. Therefore, the data is preprocessed to calculate bounding boxes around masks and stored in labels subdirectory for each validation, training and testing ground truths. Furthermore, the images are scaled down to 640 by 640 and normalised in order to increase model speed due to decreased computation on the smaller images sizes in training. The process of data preparation is done through the file [train.py](./train.py) file.
...
Inside modules.py, the create_yaml function automatically generates this file based on dataset directory structure. Below showcases the content of the yaml file:
...
The model has been trained for 50 epochs with a batch size of 16. During the training, the network learned to predict bounding boxes around lesions, hence optimizing for the detection and localization tasks. Metrics such as precision, recall, and F1-score were tracked. Periodic saving of model weights was done for evaluation purposes.
...
The model achieves a high mAP@0.5 of 0.97 and peaks to an F1-score of 0.93 at a confidence threshold of 0.43. The Precision–Recall curve shows that the model maintains high precision throughout at different recall levels, indicating good detection with very few false positives. The F1–Confidence curve further underlines a good balance between precision and recall: a broad stable region followed by a steep decline at higher confidence thresholds suggests reliable performance and well-calibrated predictions.
...
It can be seen through IoU distribution of images, that it is positively skewed. Most Images lie above the threshold of 0.8 as shown in the image. Statistically, approximately 80% images that were detected as positive, had an IoU score of over 0.8 as shown throgh the precision score. Below showcases the extended metrics:
...
Precision of 0.79 suggests that when a lesion is predicted, the model is highly accurate as well as precisely localized. The balance of recall and precision can also be used as a classification metrics as its a single class object detection task. F1-score of 0.79 reflects on the models classification results where it avoids false detection (high precision) and it's able to accurately detect true lesions (represented by the recall)
...
All the graphs/metrics for validation and training will be available in the following directory:
